...
def nomalize_for_ESC(file_name,label_path,gene_num,rate):
    data = pd.read_csv(file_name, header=None, sep=",")
    label = pd.read_csv(label_path, header=0, sep=",")
    # data_label = []
    label = np.array(label)[:, 1]
    ...
    adata.obs['cell_name'] = cell_name
    adata.var['Gene'] = gene_name
    adata = normalize(adata, copy=True, highly_genes=gene_num, size_factors=True, normalize_input=False,#False
                      logtrans_input=True)
    count = adata.X
    # a = pd.DataFrame(count).T
    # a.to_csv("./test_csv/Klein/process_data.csv")#保留高度可变基因2048的基因表达数据，normalize_input为false，方便后面计算L1和余弦相似性
    # print("generate processing data")
...
def nomalize_for_Zeisel(file_name,label_path,gene_num,rate):
    ...
    adata.obs['cell_name'] = cell_name
    adata.var['Gene'] = gene_name
    adata = normalize(adata, copy=True, highly_genes=gene_num, size_factors=True, normalize_input=True,#False
                      logtrans_input=True)
    count = adata.X
    # a = pd.DataFrame(count).T
    # a.to_csv("./test_csv/Klein/process_data.csv")#保留高度可变基因2048的基因表达数据，normalize_input为false，方便后面计算L1和余弦相似性
...
if __name__=="__main__":
    ...
    data_set = "Klein"
    imputed_file = "./results/Klein-imputed0.11.csv"
    original_file = "./test_csv/" + data_set + "/process_data.csv"
    imputation_impute_matrix(imputed_file, original_file, 0.1)
    # file_name="./test_csv/Klein/data.csv"
    # label_path= "./test_csv/Klein/label.csv"
    # nomalize_for_Zeisel(file_name, label_path, 2048, 0.1)
    ...
    #     adata,cell_type= create_adata_AD("./compare_funtion/AFGRL/AFGRL-master/results/"+data_set + "/"+function_list[i]+"-"+data_set+"-imputed.csv","./compare_funtion/AFGRL/AFGRL-master/test_csv/AD_interpretable/GSE138852_covariates.csv","./compare_funtion/AFGRL/AFGRL-master/test_csv/AD_interpretable/GSE138852_counts.csv",function_list[i],result_file,gene_list)
    #     rcParams['figure.figsize'] = 12,8
    #     sc.pp.neighbors(adata)
    #     sc.tl.umap(adata)
    # #     # 如果设置了 adata 的 .raw 属性时，下图显示了“raw”（标准化、对数化但未校正）基因表达矩阵。
    #     sc.pl.umap(adata,ncols = 2, color=["batches",function_list[i]],save = "./"+data_set+"/"+function_list[i]+".png")
