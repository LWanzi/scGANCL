...
parser.add_argument('--count_csv_path', type=str, default="D:/Python code/GANRevise1/scIGANs-master-GPU-ReviseAug/scIGANs-scIGANs_0.1.1/src/test_csv/Klein/raw-0.1s.csv",
                    help='path to counts csv file')
# parser.add_argument('--label_csv_path', type=str, default=None,
#                     help='path to labels csv file')
parser.add_argument('--label_csv_path', type=str, default="D:/Python code/GANRevise1/scIGANs-master-GPU-ReviseAug/scIGANs-scIGANs_0.1.1/src/test_csv/Klein/label1.csv",
                    help='path to labels csv file')#其中label1文件只有标签，没有标签索引，否则andata write hdf5文件会出现key为int的错误
# parser.add_argument('--save_h5ad_dir', type=str, default="./",
#                     help='dir to savings')
# parser.add_argument('--save_h5ad_dir', type=str, default="D:/Python code/scIGANs-master/scIGANs-scIGANs_0.1.1/test_data/",
#                     help='dir to savings')
parser.add_argument('--save_h5ad_dir', type=str, default="D:/Python code/GANRevise1/scIGANs-master-GPU-ReviseAug/scIGANs-scIGANs_0.1.1/src/test_csv/Klein/data",
                    help='dir to savings')
...
parser.add_argument("--select_hvg", action="store_true",default=True,
                    help="Whether select highly variable gene")

parser.add_argument("--drop_prob", type=float, default=0.0,
                    help="simulate dropout events")

parser.add_argument("--scTransform", action="store_true",default=True,
...
def preprocess_csv_to_h5ad(
    ...
    if input_h5ad_path != None and input_10X_path == None and count_csv_path == None:
        ...
        count_frame = pd.read_csv(count_csv_path, index_col=False, header=None)
        print("counts shape:{}".format(count_frame.shape))

        if label_csv_path != None:
            label_frame = pd.read_csv(label_csv_path, index_col=False, header=None)
            ...
            adata = sc.AnnData(X=count_frame.values, obs=label_frame)#在count_frame后面添加count_frame.values
